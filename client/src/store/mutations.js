...
const SET_RENDERER = (state) => {
    ...
    state.mainGraph.renderState.Renderer = Viva.Graph.View.renderer(state.mainGraph.Graph, {
        layout: layout,
...
const SET_NODE_COLOR = (state, {
    node,
    color
}) => {
    state.mainGraph.renderState.Renderer.getGraphics().getNodeUI(node).color = color
}

const SET_NODE_SIZE = (state, {
    node,
    size
}) => {
    state.mainGraph.renderState.Renderer.getGraphics().getNodeUI(node).size = size;
}
...
const SET_NODE_POSITION = (state, { nodeId, xPosition, yPosition }) => {
    state.mainGraph.renderState.layout.setNodePosition(nodeId, xPosition, yPosition)
}
...
const ADD_NODE_RULE = (state, { searchObject, searchString, type, args }) => {
    const rulesets = state.configurations.appearanceConfiguration.nodeConfiguration[type]
...
const SET_EDGE_COLOR = (state, { link, color }) => {
    state.mainGraph.renderState.Renderer.getGraphics().getLinkUI(link.id).color = color
}

const UPDATE_TOOLTIP_RULES = (state, { rules }) => {
    state.configurations.appearanceConfiguration.nodeConfiguration.tooltip = rules
}
...
function updateConfig(config, newConfig){
    let index = config.findIndex(element=>element.nodeType===newConfig.nodeType)
    if(index===-1){
