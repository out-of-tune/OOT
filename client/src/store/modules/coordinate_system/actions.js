...
function parseParameter(nodes, sortParameter) {
  const parsedNodes = nodes.map((node) => {
    const {
      data: { [sortParameter]: param, ...innerData },
      ...outerData
    } = node;
...
function sortByDataParameter(sortParameter) {
  return (a, b) =>
    a["data"][sortParameter] > b["data"][sortParameter]
      ? 1
      : b["data"][sortParameter] > a["data"][sortParameter]
      ? -1
      : 0;
}
...
const applyNodeCoordinateSystemLine = (
  { commit, rootState },
  { xOffset = 0, yOffset = 0, slope = 0, distance = 50, invertedAxis = false },
) => {
  ...
  sortedNodes.forEach((node, index) => {
    ...
    const xPosition =
      xOffset + (invertedAxis ? dimensionTwoPosition : dimensionOnePosition);
    const yPosition =
      -yOffset + (invertedAxis ? -dimensionOnePosition : -dimensionTwoPosition);
    commit("SET_NODE_POSITION", {
      nodeId: node.id,
      xPosition,
      yPosition,
...
const applyNodeCoordinateSystemMap = (
  ...
  if (nodes.length > 0) {
    ...
    if (
      !(
        isNaN(boundaries.xMin) ||
        isNaN(boundaries.yMin) ||
        isNaN(boundaries.xMax) ||
        isNaN(boundaries.yMax)
      )
    ) {
      nodes.forEach((node) => {
        const xPosition =
          rangeMap(
            node["data"][xAttributeKey],
            boundaries.xMin,
            boundaries.xMax,
            0,
            mapXLength,
        ...
        const jitteredXPosition = jitter
          ? calculateJitter(xPosition, parseFloat(mapXLength) / 200)
          : xPosition;
        const jitteredYPosition = jitter
          ? calculateJitter(yPosition, parseFloat(mapYLength) / 200)
          : yPosition;
...
const applyNodeCoordinateSystem = (
  { dispatch },
  { nodeLabel, layoutTypeOptions, layoutType },
) => {
  switch (layoutType) {
    case "line":
...
const applyCoordinateSystems = ({ rootState, dispatch }) => {
...
function commitConfigurationToState(rootState, commit, configuration) {
  const labels = rootState.configurations.layoutConfiguration.map(
    (entry) => entry.nodeLabel,
  );
...
const setCoordinateSystemConfiguration = (
  { rootState, commit },
  configuration,
) => {
