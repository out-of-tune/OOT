...
describe("getSongSamples",()=>{
    ...
    it("updates artist node data",async ()=>{
        SpotifyService.getSongSamplesFromArtist.mockResolvedValue({tracks:["someData"]})
        await getSongSamples({commit, rootState, dispatch}, node)
        expect(commit).toHaveBeenCalledWith("ADD_NODE_DATA", {node:node, data: {tracks:["someData"]}})
    })
    ...
    it("calls getSongsFromAlbum when node is album",async ()=>{
        node = {id:"album/1", data:{label: "album"}}
        SpotifyService.getSongSamplesFromArtist.mockResolvedValue({items:["someData"]})
        await getSongSamples({commit, rootState, dispatch}, node)
...
describe("insertInQueue",()=>{
    ...
    it("commits SET_CURRENT_SONG",()=>{
        ...
        expect(commit).toHaveBeenCalledWith("INSERT_IN_QUEUE", {song, position:1})
    })
})
describe("addToQueue",()=>{
    let commit
...
describe("playSong",()=>{
    ...
    it("adds song to first place when queue is empty",()=>{
        ...
        expect(dispatch).toHaveBeenNthCalledWith(1,"insertInQueue", {song:{el:"one dummy element"}, position:0})
        expect(dispatch).toHaveBeenNthCalledWith(2,"playAtIndexInQueue", 0)
    })
    it("adds song after the song that is currently playing",()=>{
        state.queue = [{el:"one dummy element"},{el:"two dummy element"},{el:"three dummy element"}]
        state.queueIndex = 1
        playSong({dispatch,state},{el:"between to and three dummy element"})
        expect(dispatch).toHaveBeenNthCalledWith(1,"insertInQueue", {song:{el:"between to and three dummy element"}, position:2})
...
describe("playAtIndexInQueue",()=>{
    ...
    it("it plays the song and sets the queueIndex",()=>{
        ...
        expect(commit).toHaveBeenNthCalledWith(1,"SET_CURRENT_SONG",{el:"two dummy element"})
