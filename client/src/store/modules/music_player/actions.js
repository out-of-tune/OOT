...
const getSongSamples = async ({ commit, rootState, dispatch }, node) => {
  ...
  const tracks =
    node.data.label === "artist" ? result[0].tracks : result[0].items;
...
const playSong = ({ dispatch, state }, song) => {
  const currentQueuePosition =
    state.queue.length == 0 ? 0 : state.queueIndex + 1;
...
const retrieveFullSongData = async (dispatch, rootState, node) => {
  ...
  return {
    node: node,
    data: { ...result.tracks[0], images: result.tracks[0].album.images },
  };
};
