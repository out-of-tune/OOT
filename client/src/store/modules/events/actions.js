...
const mouseClickFunctionality = async ({ commit, dispatch, rootState }, node) => {
...
const mouseMoveFunctionality = ({ rootState, commit, dispatch }, node) => {
    if (node && rootState.selection.selectedNodes.length > 0 && rootState.events.moveOriginPosition) {
        if (rootState.mainGraph.renderState.isRendered) {
            commit("PAUSE_RENDERING")
        }
        dispatch("moveSelection", { originNode: node, nodesWithPositionToMove: rootState.events.originNodePositions, oldOriginPosition: rootState.events.moveOriginPosition })
    }
}
const mouseDownFunctionality = ({ rootState, commit }, node) => {
    if (rootState.selection.selectedNodes.filter((n) => n.id == node.id).length == 1) {
        commit("SET_MOUSE_PAUSED", !rootState.mainGraph.renderState.isRendered)
        commit("SET_MOVE_ORIGIN_POSITION", getNodePosition({ graphics: rootState.mainGraph.renderState.Renderer.getGraphics(), node }))
        const nodesWithPosition = rootState.selection.selectedNodes.map((node) => {
            const pos = getNodePosition({ graphics: rootState.mainGraph.renderState.Renderer.getGraphics(), node })
...
const keyUpFunctions = ({dispatch, commit, rootState, state}, e)=>{        
    ...
    else if((e.which === SHIFT || e.which === CTRL) && state.multiSelectOverlay){
...
const keyDownFunctions = ({rootState, commit, dispatch, state}, e)=>{
    if (e.which===SHIFT && !state.keysdown[SHIFT] && !state.multiSelectOverlay) {
        commit("SET_WAS_PAUSED", !rootState.mainGraph.renderState.isRendered)
        if(rootState.mainGraph.renderState.isRendered){
            commit("PAUSE_RENDERING")
        }
        commit("SET_MULTI_SELECT_OVERLAY", startMultiSelect({ 
            onAreaSelectedCallback: (area)=>dispatch("handleAreaSelected", {area}), 
...
const initSelectionEvents = ({ dispatch }) => {
    ...
    document.getElementsByClassName("graph-overlay")[0].addEventListener('mousedown', (e)=>{dispatch("globalMouseDownFunctions",e)})
    document.getElementsByClassName("graph-overlay")[0].addEventListener('keyup', (e)=>{dispatch("keyUpFunctions",e)})

    document.addEventListener('mouseup', (e)=>dispatch("globalMouseUpFunctions", e))
    document.getElementById("graphContainer").addEventListener("keydown", keyDownFunctions);
    document.getElementById("graphContainer").addEventListener("keyup", (e)=>{dispatch("keyUpFunctions",e)});
}
