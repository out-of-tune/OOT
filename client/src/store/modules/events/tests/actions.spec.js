...
describe("mouseMoveFunctionality", ()=>{
    ...
    it("moves the selection", ()=>{
        mouseMoveFunctionality({rootState, commit, dispatch}, {id: "1"})
        expect(dispatch).toHaveBeenCalledWith("moveSelection", {originNode: {id: "1"},  nodesWithPositionToMove: rootState.events.originNodePositions, oldOriginPosition: rootState.events.moveOriginPosition})
    })
...
describe("mouseDownFunctionality", ()=>{
    ...
    beforeEach(()=>{
        ...
        getNodePosition.mockReturnValueOnce({x: 10, y: 10}).mockReturnValueOnce({x: 10, y: 15}).mockReturnValue({x: 10, y:10})
    })
    ...
    it("sets the start position of the clicked node", ()=>{
        mouseDownFunctionality({rootState, commit}, {id: "2"})
        expect(commit).toHaveBeenCalledWith("SET_MOVE_ORIGIN_POSITION", {x: 10, y: 15})
    })
    it("sets the start position of the selected nodes", ()=>{
        mouseDownFunctionality({rootState, commit}, {id: "2"})
        expect(commit).toHaveBeenCalledWith("SET_AFFECTED_NODES_ORIGIN_POSITION", [{node: {id: "1"}, position: {x: 10, y: 15}}, {node: {id: "2"}, position: {x: 10, y: 10}}])
    })
})
...
describe("mouseClickFunctionality", () => {
    ...
    it("calls expand funtion on mouse click", async() => {
        ...
        expect(dispatch).toHaveBeenCalledWith("expandAction", {nodes: [clickedNode]})
    })
...
describe("changePinStatus",()=>{
    ...
    it("calls PIN_NODE when a node is unpinned",()=>{
        ...
        changePinStatus({commit,rootState},{id:"Werk/1",data:{label:"Werk"}})
        expect(commit).toHaveBeenCalledWith("PIN_NODE",{id:"Werk/1",data:{label:"Werk"}})
    })
    it("calls UNPIN_NODE when a node is pinned",()=>{
        getPinnedState.mockReturnValue(true)
        changePinStatus({commit,rootState},{id:"Werk/1",data:{label:"Werk"}})
        expect(commit).toHaveBeenCalledWith("UNPIN_NODE",{id:"Werk/1",data:{label:"Werk"}})
    })
})
describe("keyUpFunctions",()=>{
    ...
    it("pins selected nodes when p is pressed", ()=>{
        e.which = P
        keyUpFunctions({dispatch, commit, rootState, state},e)
        expect(dispatch).toHaveBeenCalledWith("pinNodes", rootState.selection.selectedNodes)
    })
    it("unpins selected nodes when u is pressed", ()=>{
        e.which = U
        keyUpFunctions({dispatch, commit, rootState, state},e)
        expect(dispatch).toHaveBeenCalledWith("unpinNodes", rootState.selection.selectedNodes)
    })
...
describe("keyDownFunctions",()=>{
    ...
    it("starts multi select when shift is pressed", ()=>{
        e.which = SHIFT
        keyDownFunctions({dispatch, commit, rootState, state},e)
        expect(commit).toHaveBeenCalledWith("SET_MULTI_SELECT_OVERLAY", {id:"1"})
    })
...
describe("globalMouseUpFunctions",()=>{
    ...
    it("finishes selection when shift is pressed", ()=>{
        state.keysdown[CTRL] = true
        globalMouseUpFunctions({dispatch, commit, state},e)
        expect(dispatch).toHaveBeenCalledWith("selectionFinished", {addToSelection: true})
    })
...
describe("resizeGraphContainer", ()=>{
    ...
    it("resizes graph container", ()=>{
        ...
        expect(commit).toHaveBeenCalledWith("RESIZE_GRAPH",{width: 100, height: 100})
    })
})
