...
const storeGraph = ({ rootState, dispatch, state, commit }, name) => {
    ...
    try {
        ...
        const graphNames = state.storedGraphNames.includes(name) ? state.storedGraphNames : [...state.storedGraphNames, name]
        commit('SET_STORED_GRAPH_NAMES', graphNames)
        dispatch('setSuccess', 'save complete')
    }
    catch(e){
        dispatch('setError', new Error('Graph could not be saved, please download it.'))
    }
}
...
const removeGraphFromIndexedDb = async ({commit, dispatch, state}, name)=>{
    try{
        ...
        const graphNames = state.storedGraphNames.filter((graphName)=>graphName!==name)
        commit('SET_STORED_GRAPH_NAMES', graphNames)
