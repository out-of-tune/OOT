...
import { handleGraphqlTokenError, handleTokenError } from '@/assets/js/TokenHelper.js'
import { getAllNodes, getAllLinks } from '@/assets/js/graphHelper.js'
import { checkNodesExistence, mergeGraphQlQueries } from '@/assets/js/graphQlHelper.js'
...
function lookUpEdgeTypes(rootState, connections){
    return rootState.schema.edgeTypes.filter(
        edgeType => connections.includes(edgeType.label)
    )
}
function getDirectedConnections(connections, nodeLabel){
    return connections.map(connection => 
        connection.inbound.from === nodeLabel ? connection.inbound : connection.outbound
    )
}
...
function lookUpNodeLabel(schema, key, clickedNode) {
    return getMatchingEdgeTypes(schema, key, clickedNode).map(edgeType => {
        return key === edgeType.inbound.connectionName ? edgeType.inbound.to
            : key === edgeType.outbound.connectionName ? edgeType.outbound.to
                : null
    })[0]
}
...
function getMatchingEdgeTypes(schema, key, clickedNode) {
    return schema.edgeTypes.filter(edgeType => {
        return (key === edgeType.inbound.connectionName && clickedNode.data.label === edgeType.inbound.from) 
        || (key == edgeType.outbound.connectionName && clickedNode.data.label === edgeType.outbound.from)
    })
}
...
function extractLinksFromResponse(responseData, schema, clickedNode) {
    return Object.keys(responseData).flatMap(key => {
        const lookedUpConnectionLabel = lookUpConnectionLabel(schema, key, clickedNode)
        return responseData[key].map(node => {
            return ({
...
function getConnections(rootState, node, expandConfiguration = null){
    const config = expandConfiguration ? expandConfiguration: rootState.configurations.actionConfiguration.expand
    ...
    const connections = getDirectedConnections(matchedConnections, node.data.label)
    return connections
}

function getGraphQlConnectionsToQuery(rootState, graphQlConnections){
    return rootState.schema.nodeTypes.flatMap(nodeType => {
        return graphQlConnections.filter(graphQlConnection =>
            graphQlConnection.to === nodeType.label
        ).map(graphQlConnection => {
            return { connectionName: graphQlConnection.connectionName, attributes: nodeType.attributes }
        })
    })
}
...
async function getGraphQlResult(rootState, nodes, dispatch){
    ...
    const endresult = data.map((result, index)=>queryResolver(result[0], rootState.schema, nodes[index].node))
    return endresult
}

async function handleGetArtistsByIdError(token, chunk){
    const boundGetArtistsById = (SpotifyService.getArtistsById).bind(SpotifyService)
    try {
        const data = await handleTooManyRequestsError(boundGetArtistsById, [token, chunk])
        return data
    }
    catch(error){
...
async function getArtistsFromAlbum(connections, token){
    ...
    const result = (await Promise.all(promises))
        ...
        .map((artist, index)=>artist ? artist : { id: artistSids[index], error: new Error("artist is null")})
...
async function handleGetFullSongDataError(token, chunk){
    ...
    const boundGetAlbumFromSong = (SpotifyService.getFullSongData).bind(SpotifyService)
    try {
        const data = await handleTooManyRequestsError(boundGetAlbumFromSong, [token, sids])
        return data
    }
    catch(error){
        return { tracks: chunk.map(connection=>({ error: { connection, reason: error } })) }
    }
}
...
async function getAlbumFromSong(connections, token){
    ...
    const result = (await Promise.all(promises))
        ...
        .map((track, index)=>{
            ...
            : { items: [], error: { connection: connections[index], reason: new Error('track is null') } }
        })
    return result
}

async function handleGetAlbumsFromArtistError(token, connection){
    const boundGetAlbumsFromArtist = (SpotifyService.getAlbumsFromArtist).bind(SpotifyService)
    try {
        const data = await handleTooManyRequestsError(boundGetAlbumsFromArtist, [token, connection.node.data.sid])
        return data
    }
    catch(error){
...
async function getAlbumsFromArtist(connections, token){
    return await Promise.all(connections.map( connection => handleGetAlbumsFromArtistError(token, connection)))
}

async function handleGetSongsFromAlbumError(token, connection){
    const boundGetSongsFromAlbum = (SpotifyService.getSongsFromAlbum).bind(SpotifyService)
    try {
        const data = await handleTooManyRequestsError(boundGetSongsFromAlbum, [token, connection.node.data.sid])
        return data
    }
    catch(error){
        return { items: [], error: { connection, reason: error } }
...
async function getSongsFromAlbum(connections, token){
    return await Promise.all(connections.map( connection => handleGetSongsFromAlbumError(token, connection)))
}

function convertGetArtistFromAlbumResult(artistResults, connections){
    return connections.map(connection=>{
        const items = connection.node.data.artists.map(artist=>artistResults[artist.id])
...
async function fetchData(categorizedConnections, token){
    ...
    const getAlbumFromSongResult = cc.getAlbumFromSong.length>0 ? getAlbumFromSong(cc.getAlbumFromSong, token) : null
    const getAlbumsFromArtistResult = cc.getAlbumsFromArtist.length>0 ? getAlbumsFromArtist(cc.getAlbumsFromArtist, token) : null
    const getArtistsFromAlbumResult = cc.getArtistsFromAlbum.length>0 ? await getArtistsFromAlbum(cc.getArtistsFromAlbum, token) : null
    const getSongsFromAlbumResult = cc.getSongsFromAlbum.length>0 ? getSongsFromAlbum(cc.getSongsFromAlbum, token) : null

    const convertedArtistsFromAlbumResult = getArtistsFromAlbumResult ? convertGetArtistFromAlbumResult(getArtistsFromAlbumResult, cc.getArtistsFromAlbum) : null
    const result = {
...
function categorizeConnections(nodes){
    const connections = nodes.flatMap(n=>n.connections.map(c=>({ ...c, node: n.node })))
    const getConnectionCategory = (from, to) => connections.filter(c=>c.from===from && c.to===to)
    return {
...
async function getSpotifyData(rootState, nodes, dispatch){
    ...
    const data = await handleTokenError(fetchData, [ categorizedConnections ], dispatch, rootState)
 ...
 */
async function generateNodesAndLinks(data, connections, schema, dispatch, rootState){
    if(data){
        ...
        const dbNodes = (connections[0].to === "artist" && itemsWithConnections.length>0) 
            ? await (getDbNodes(itemsWithConnections, "artist", schema, dispatch, rootState)) : null

        const result = itemsWithConnections.map((data, index)=>{
            const node = dbNodes && dbNodes[index] ? dbNodes[index]: buildNode(data.connection.to, data.item)
            return {
                node,
                link: buildLink(node.id, data.connection.node, schema, data.connection.connectionName)
            }
        })
        return result
    }
...
async function getSpotifyResult(rootState, nodes, dispatch){
    ...
    const nodesAndLinks = (await Promise.all([
        ...(await generateNodesAndLinks(data.getArtistsFromAlbum, categorizedConnections.getArtistsFromAlbum, schema, dispatch, rootState)),
        ...(await generateNodesAndLinks(data.getSongsFromAlbum, categorizedConnections.getSongsFromAlbum, schema, dispatch, rootState)),
        ...(await generateNodesAndLinks(data.getAlbumsFromArtist, categorizedConnections.getAlbumsFromArtist, schema, dispatch, rootState)),
        ...(await generateNodesAndLinks(data.getAlbumFromSong, categorizedConnections.getAlbumFromSong, schema, dispatch, rootState))
    ])).reduce((acc, prev)=>{
        return { nodes: [...acc.nodes, prev.node], links: [...acc.links, prev.link]}
    }, {nodes: [], links: []})

    return [nodesAndLinks]
}

const getNodesWithConnections = (endpoint, nodes, rootState, expandConfiguration) => {
    return nodes.map(node=>{
        return {
            node: node,
            connections: filterConnections(getConnections(rootState, node, expandConfiguration), endpoint)
        }
    }).filter(node=>node.connections.length>0)
}

const addResults = results => results.reduce((prev, curr)=>{
    return {nodes: [...prev.nodes, ...curr.nodes], links: [...prev.links, ...curr.links]}
}, {nodes: [], links: []})
 ...
 */
const expandAction = async ({ rootState, dispatch }, { nodes, expandConfiguration = null, tries = 0 }) => {
    dispatch('setFailedExpandedConnections', [])
    dispatch('addPendingRequest')
    const getResult = endpoint => {
        const nodesWithConnections = getNodesWithConnections(endpoint, nodes, rootState, expandConfiguration)
    ...
    const result = addResults([ ...(await getResult("graphQl")), ...(await getResult("spotify")) ])
    dispatch('removePendingRequest')
    dispatch('addToGraph', result)
    if(rootState.expand.failedExpandedConnections.length>0 && tries<3){
        dispatch('setInfo', rootState.expand.failedExpandedConnections.length + " requests failed. Trying again in 10 seconds.")
        await sleep(10000)
        const nodes = rootState.expand.failedExpandedConnections.map(connection=>connection.node)
        dispatch('expandAction', { nodes, expandConfiguration, tries: tries+1 })
    }
    else if(rootState.expand.failedExpandedConnections.length>0){
        dispatch('setInfo', rootState.expand.failedExpandedConnections.length + " requests failed.")
    }
}
...
const addToGraph = ({ commit, dispatch, rootState }, { nodes, links }) => {
    ...
    const newLinks = links.filter(link=>{
        const existingLink = rootState.mainGraph.Graph.getLink(link.fromId, link.toId)
        return !!existingLink ? !existingLink.linkTypes.includes(link.linkTypes[0]) : true
    })

    dispatch('addChange', {
        data: { nodes: newNodes, links: newLinks }, type: "add"
    })
...
function setNodesToNearPosition(rootState, commit, links) {
    ...
    links.forEach((link, index)=>{
        ...
        const sumPosition = positions.reduce((prev, curr)=>{
            return { x: prev.x + curr.x, y: prev.y + curr.y }
        }, { x: 0, y: 0 })
        const position = { x: sumPosition.x/positions.length, y: sumPosition.y/positions.length }
...
const addFailedExpandedConnections = ({ commit, state }, connections) => {
    commit('SET_FAILED_EXPANDED_CONNECTIONS', [...state.failedExpandedConnections, connections])
}
