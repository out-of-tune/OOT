...
describe("startGraphQlSearch", () => {
  ...
  it("errors when graphql returns an error", async () => {
    rootState.searchObject = {
      ...
      nodeType: "Auftrag",
      attributes: [
        { attributeSearch: "some", operator: "=", attributeData: "garbage" },
      ],
    };
  ...
  it("adds nodes from graphql endpoint ", async () => {
    rootState.searchObject = {
      ...
      attributes: [
        {
          attributeSearch: "some",
          operator: " LIKE ",
          attributeData: "nice value1",
        },
      ],
    };
    handleGraphqlTokenError.mockReturnValue({
      artist: [
        { id: "artist/1", money: 33, volume: 12 },
        { id: "artist/2", money: 33, volume: 12 },
      ],
    });
    await startGraphQlSearch({ commit, rootState, dispatch });
    expect(commit).toHaveBeenCalledWith("ADD_TO_GRAPH", {
      nodes: [
        { id: "artist/1", data: { label: "artist", money: 33, volume: 12 } },
        { id: "artist/2", data: { label: "artist", money: 33, volume: 12 } },
      ],
      links: [],
    });
  });
