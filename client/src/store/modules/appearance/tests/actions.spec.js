...
describe("storeColors", () => {
  ...
  beforeEach(() => {
    ...
    nodes = [{ id: "Auftrag/1", data: { label: "Auftrag" } }, { id: "Auftrag/2", data: { label: "Auftrag" } }]
  ...
  it("saves link and node colors to state", () => {
    ...
    expect(commit).toHaveBeenCalledWith("SET_COLORS", { links: [{ color: "000000ff", link: { from: "Auftrag/1", to: "Auftrag/2" } }], nodes: [{ color: "ffffffff", node: { id: "Auftrag/1", data: { label: "Auftrag" } } }, { color: "ffffffff", node: { id: "Auftrag/2", data: { label: "Auftrag" } } }] })
  })
})
...
describe("loadColors", () => {
  ...
  beforeEach(() => {
    ...
    state = {
      colors: {
        links: [{ color: "000000ff", link: { from: "Auftrag/1", to: "Auftrag/2" } }],
        nodes: [{ color: "fffaaaff", node: { id: "Auftrag/1", data: { label: "Auftrag" } } }, { color: "ffffffff", node: { id: "Auftrag/2", data: { label: "Auftrag" } } }]
      }
    }
    nodes = [{ id: "Auftrag/1", data: { label: "Auftrag" } }, { id: "Auftrag/2", data: { label: "Auftrag" } }]
    links = [{ from: "Auftrag/1", to: "Auftrag/2" }]
  ...
  it("sets node colors", () => {
    ...
    expect(commit).toHaveBeenCalledWith("SET_NODE_COLOR", { node: "Auftrag/1", color: "fffaaaff" })
    expect(commit).toHaveBeenCalledWith("SET_NODE_COLOR", { node: "Auftrag/2", color: "ffffffff" })
  })
  it("sets link colors", () => {
    loadColors({ state, dispatch, commit })
    expect(commit).toHaveBeenCalledWith("SET_EDGE_COLOR", { color: "000000ff", link: { from: "Auftrag/1", to: "Auftrag/2" } })
  })
  it("rerenders graph", () => {
    loadColors({ state, dispatch, commit })
...
describe("highlight", () => {
  ...
  beforeEach(() => {
    ...
    allLinks = [{ id: "1to2", fromId: "Auftrag/1", toId: "Auftrag/2" }, { id: "2to3", fromId: "Auftrag/2", toId: "Auftrag/3" }]
    allNodes = [{ id: "Auftrag/1", data: { label: "Auftrag" }, links: [allLinks[0]] },
    ...
    allNodes.forEach(node => rootState.mainGraph.Graph.addNode(node.id, node.data))
    allLinks.forEach(link => rootState.mainGraph.Graph.addLink(link.fromId, link.toId))
  })
  it("sets all not highlighted nodes transparent", () => {
    highlight({ rootState, dispatch, commit }, allNodes[0])
    expect(commit).toHaveBeenCalledWith("SET_NODE_COLOR", { node: "Auftrag/3", color: parseInt("77777733", 16) })
  })
  it("sets all highlighted nodes solid", () => {
    highlight({ rootState, dispatch, commit }, allNodes[0])
    expect(dispatch).toHaveBeenCalledWith("setNodeOpacity", { node: { id: "Auftrag/2" }, opacity: "ff" })
    expect(dispatch).toHaveBeenCalledWith("setNodeOpacity", { node: { id: "Auftrag/1" }, opacity: "ff" })
  })
  it("sets all not highlighted nodes transparent", () => {
    highlight({ rootState, dispatch, commit }, allNodes[0])
    expect(commit).toHaveBeenCalledWith("SET_EDGE_COLOR", { link: allLinks[1], color: parseInt("77777733", 16) })
  })
  it("sets all highlighted links solid", () => {
    highlight({ rootState, dispatch, commit }, allNodes[0])
    expect(dispatch).toHaveBeenCalledWith("setLinkOpacity", { link: { id: allLinks[0].id }, opacity: "ff" })
  })
  it("rerenders graph", () => {
    highlight({ rootState, dispatch, commit }, allNodes[0])
    expect(dispatch).toHaveBeenCalledWith("rerenderGraph")
  ...
  it("only highlights node when it has no connections", () => {
    ...
    expect(dispatch).toHaveBeenCalledWith("setNodeOpacity", { node: { id: "Auftrag/3" }, opacity: "ff" })
  })
})
