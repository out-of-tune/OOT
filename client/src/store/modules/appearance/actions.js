...
const clusterNodes = ({ rootState, commit }) => {
  ...
  function visitCluster(clusterNodes, clusterClass) {
    ...
    const getHexColorString = () => {
      return getRandomNumber(0x000000, 0xffffff).toString(16) + "ff";
    };
...
const toggleEdgeVisibility = ({ rootState, commit }) => {
...
const switchRendering = ({ rootState, commit }) => {
...
const highlight = ({ rootState, commit, dispatch }, node) => {
  ...
  const nodes = getAllNodes(rootState).filter(
    (node) => !nodeIds.includes(node.id),
  );
  const links = getAllLinks(rootState).filter(
