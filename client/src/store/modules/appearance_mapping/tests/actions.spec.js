...
describe("setNodeSizeRule", () => {
  ...
  it('dispatches setNodeSizeMapped when sizeType is map', () => {
    ...
    expect(dispatch).toHaveBeenCalledWith('setNodeSizeMapped', {
      nodes: selectedNodes, attribute: "name", minMapValue: 10, maxMapValue: 50
    })
  })
...
describe('applyNodeConfiguration', () => {
  ...
  it('dispatches default action when rules are empty', () => {
    ...
    applyNodeConfiguration({ dispatch, rootState }, {
      configuration,
      ruleAction,
      defaultAction
    })
    expect(dispatch).toHaveBeenCalledWith('setNodeColorDefault', [nodes[0], nodes[2]])
  })
})
...
describe("addRule", () => {
  ...
  it("adds a valid color rule", () => {
    ...
    type = "color"
    let color = "ffffffff"
    addRule({ commit, rootState, dispatch }, { type, searchObject: rootState.searchObject, searchString: rootState.searchString, color })
  ...
  it("adds a valid size map rule", () => {
    ...
    type = "size"
    let size = 12
    addRule({ commit, rootState, dispatch }, { type, searchObject: rootState.searchObject, searchString: rootState.searchString, sizeType: "map" })
    expect(commit).toHaveBeenCalledWith("ADD_NODE_RULE", {
      searchObject: {
        "valid": true,
        "errors": [],
  ...
  it("errors when no attribute is given while adding a map rule", () => {
    ...
    let size = 12
    addRule({ commit, rootState, dispatch }, { type, searchObject: rootState.searchObject, searchString: rootState.searchString, sizeType: "map" })
    expect(dispatch).toHaveBeenCalledWith("setError", new Error("An attribute has to be chosen to add a map rule"))
  })
...
describe("applyEdgeColorConfiguration", () => {
  ...
  it('commits SET_EDGE_COLOR for all links', () => {
    ...
    let expectedColors = [
      "00ffffff", "00ffffff", "00fffff8"
    ]
    expect(commit).toHaveBeenNthCalledWith(1, "SET_EDGE_COLOR", { link: links[0], color: parseInt(expectedColors[0], 16) })
    expect(commit).toHaveBeenNthCalledWith(2, "SET_EDGE_COLOR", { link: links[1], color: parseInt(expectedColors[1], 16) })
    expect(commit).toHaveBeenNthCalledWith(3, "SET_EDGE_COLOR", { link: links[2], color: parseInt(expectedColors[2], 16) })
  })
})
...
describe("setConfiguratedEdgeColor", () => {
  ...
  it('commits SET_EDGE_COLOR for all links', () => {
    setConfiguratedEdgeColor({ rootState, commit }, {
      fromId: "a1",
      toId: "a2",
      id: "a1👉 a2",
      linkTypes: ["a"]
    ...
    expect(commit).toHaveBeenNthCalledWith(1, "SET_EDGE_COLOR", { link: links[0], color: parseInt(expectedColor, 16) })
  })
})
...
describe("markClickedNodes", ()=>{
  ...
  it("sets the node color", ()=>{
    ...
    expect(commit).toHaveBeenCalledWith("SET_NODE_COLOR", {node: "1", color: parseInt("eeeeeeff", 16)})
  })
...
describe("updateRuleset", ()=>{
  ...
  it("updates node ruleset", ()=>{
    ...
    expect(commit).toHaveBeenCalledWith("UPDATE_NODE_RULESET", {rules: ruleset, nodeLabel, type})
  })
})
