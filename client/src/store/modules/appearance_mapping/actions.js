import { getNodesByLabel, getNodeColor, searchGraph, getAllNodes } from '@/assets/js/graphHelper'
...
const getAvg = arr => arr.reduce((previous, current, index) => {
    return (index === arr.length - 1)
        ? (previous + current) / arr.length
        : previous + current
})

const extractColorPartValue = (color, positions) => parseInt(color.substring(...positions), 16)
const getColorPartValues = (colorStrings, positions) => colorStrings.map(color => extractColorPartValue(color, positions))
const getColorPartAvg = (colorStrings, positions) => getAvg(getColorPartValues(colorStrings, positions))
...
function colorToString(color) {
    const formatColorPart = part => Math.abs(part).toString(16).padStart(2, "0")
    return formatColorPart(color.red) + formatColorPart(color.green) + formatColorPart(color.blue) + formatColorPart(color.opacity)
}

function getLinkColor(link, configuration) {
    const colorStrings = link.linkTypes.flatMap(type => {
        return configuration.filter(rule => rule.edgeLabel === type)
    }).map(rule => rule.color)
    return colorStrings.length ? colorToString(getMixedColor(colorStrings))
    : "ffffffff"
}

const setNodeColorDefault = ({
    commit
}, nodes) => {
    nodes.forEach(node => commit("SET_NODE_COLOR", {
        node: node.id,
...
const setNodeColorRule = ({ commit, rootState }, { rule, nodes }) => {
    ...
    affectedNodes.forEach(node => commit("SET_NODE_COLOR", {
        node: node.id,
...
const setNodeSizeRule = ({ commit, rootState, dispatch }, { rule, nodes }) => {
    if (rule.sizeType === "compare"){
        const affectedNodes = searchGraph(rule.searchObject, rootState, nodes)
        affectedNodes.forEach(node => commit("SET_NODE_SIZE", {
            node: node.id,
            size: rule.size
        }))
    }
        ...
        dispatch('setNodeSizeMapped', {
            nodes, attribute: rule.searchObject.attributes[0].attributeSearch, minMapValue: rule.min, maxMapValue: rule.max
        })
    }
}

const applyNodeConfiguration = ({ rootState, dispatch }, { configuration, ruleAction, defaultAction, nodes=[] }) => {
    ...
    const groupedNodes = affectedNodes.reduce(function(rv, current) {
        (rv[current["data"]["label"]] = rv[current["data"]["label"]] || []).push(current);
        return rv;
    }, {})
    ...
    configuration.forEach(nodeType => {
        if(groupedNodes[nodeType.nodeLabel]){
            if (nodeType.rules.length > 0){
                nodeType.rules.forEach(rule => dispatch(ruleAction, { rule, nodes: groupedNodes[nodeType.nodeLabel]}))
            }
            else { // node.label?
...
const markClickedNodes = ({rootState, commit})=>{
    ...
    clickedKeys.forEach((nodeId)=>{
        if(rootState.mainGraph.Graph.getNode(nodeId)){
            commit("SET_NODE_COLOR", {node: nodeId, color: parseInt("eeeeeeff", 16)})
        }
    })
}

const applyNodeColorConfiguration = ({ rootState, dispatch }, nodes=[]) => {
    const configuration = rootState.configurations.appearanceConfiguration.nodeConfiguration.color
...
const applyNodeSizeConfiguration = ({ rootState, dispatch }, nodes=[]) => {
    const configuration = rootState.configurations.appearanceConfiguration.nodeConfiguration.size
    dispatch('applyNodeConfiguration', {
        configuration,
...
const setNodeSizeMapped = ({ commit }, { nodes, attribute, minMapValue, maxMapValue }) => {
    const validNodes = nodes.filter(node=>!isNaN(parseFloat(node.data[attribute])))
    if(validNodes.length>0){
        ...
        if(maxInSet!=minInSet){
            validNodes.forEach(node => {
                ...
                const size = rangeMap(value, minInSet, maxInSet, parseFloat(minMapValue), parseFloat(maxMapValue))
...
const applyEdgeColorConfiguration = ({ rootState, commit, dispatch }) => {
    const configuration = rootState.configurations.appearanceConfiguration.edgeConfiguration.color
...
const addRule = ({commit, rootState, dispatch}, {type, searchObject, searchString, ...args}) => {
            ...
            if(type === "size" && args["sizeType"] === "map"){
                if(searchObject.attributes.length>0){
                    commit('ADD_NODE_RULE', {
                        searchObject, searchString, type, args
                    })
                    dispatch('setSuccess', "rule added")
                }
                else {
                    dispatch('setError', new Error('An attribute has to be chosen to add a map rule'))
                }
            } 
            else {
                commit('ADD_NODE_RULE', {
