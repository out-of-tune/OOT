...
function getEdgeConfigurationForNode(rootState, node) {
  const collapseConfigurations =
    rootState.configurations.actionConfiguration.collapse;
  const edges = collapseConfigurations
    .filter(
      (collapseConfiguration) =>
        collapseConfiguration.nodeType == node.data.label,
    )
...
function getLinksToRemove(nodesWithLink, edgeTypesToRemove) {
  return nodesWithLink
    .filter((nodeWithLink) => {
      return (
        nodeWithLink.link.linkTypes.filter(
          (linkType) => !edgeTypesToRemove.includes(linkType),
        ).length == 0
      );
    })
    .map((nodeWithLink) => {
...
function getLinksToUpdate(nodesWithLink, edgeTypesToRemove) {
  return nodesWithLink
    ...
    .map((nodeWithLink) => {
      const { linkTypes, ...data } = nodeWithLink.link;
      const updatedLinkTypes = nodeWithLink.link.linkTypes.filter(
        (linkType) => !edgeTypesToRemove.includes(linkType),
      );
      return {
        ...data,
...
function getNodesToRemove(rootState, nodesWithLink, edgeTypesToRemove) {
  return nodesWithLink
    .filter((nodeWithLink) => {
      const isConnectedLinkRemoved =
        nodeWithLink.link.linkTypes.filter(
          (linkType) => !edgeTypesToRemove.includes(linkType),
        ).length == 0;
      const hasNodeConnectionsLeft =
        rootState.mainGraph.Graph.getLinks(nodeWithLink.node.id).length == 1;
...
const collapseAction = ({ commit, rootState, dispatch }, node) => {
  ...
  const changedLinks = [
    ...itemsToChange.linksToRemove,
    ...itemsToChange.linksToUpdate,
  ];
