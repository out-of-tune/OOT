...
const importConfiguration = ({ dispatch }, unverifiedConfigurationString) => {
  try {
    const validator = new Validator();
    const unverifiedConfiguration = JSON.parse(unverifiedConfigurationString);
    const checkedConfiguration = validator.validate(
...
const removeConfigurationFromIndexedDb = async (
  { commit, dispatch, state },
  name,
) => {
  try {
    await IndexedDbService.deleteConfiguration(name);
    const configurationNames = state.storedConfigurationNames.filter(
