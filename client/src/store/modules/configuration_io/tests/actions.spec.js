...
describe("importConfiguration", () => {
    ...
    it("doesn't load a json file with wrong data", () => {
        ...
        expect(dispatch).toHaveBeenCalledWith("setError", new Error("error at instance\nerror at instance"))
    })
    ...
    expect(dispatch).toHaveBeenCalledWith("setError",new SyntaxError("Unexpected token < in JSON at position 0"))
    })
})
...
describe("storeConfiguration", () => {
    ...
    it("calls saveIndexedDb", () => {
        ...
        expect(IndexedDbService.saveConfiguration).toHaveBeenCalledWith("abc", rootState.configurations)
    })
    it("adds name to stored configuration name list if it is not there yet", () => {
        storeConfiguration({ rootState, dispatch, commit, state }, "abc")
        expect(commit).toHaveBeenCalledWith("SET_STORED_CONFIGURATION_NAMES", ["MamboNo1Config", "abc"])
    })
    it("doesn't add name if string already exists", () => {
        storeConfiguration({ rootState, dispatch, commit, state }, "MamboNo1Config")
        expect(commit).toHaveBeenCalledWith("SET_STORED_CONFIGURATION_NAMES", ["MamboNo1Config"])
    })
    it("dispatches an error message when IndexedDb errors", () => {
      IndexedDbService.saveConfiguration.mockImplementationOnce(new Error("A error"))
...
describe("loadConfigurationFromIndexedDb", () => {
    ...
    it(("calls loadGraph"), async () => {
        IndexedDbService.getConfiguration = jest.fn()
        IndexedDbService.getConfiguration.mockReturnValue({ id: "A configuration" })
        await loadConfigurationFromIndexedDb({ dispatch }, "conf1")
        expect(dispatch).toHaveBeenCalledWith("loadConfiguration", { id: "A configuration" })
    })
    ...
    it(("sets error message when IndexedDb errors"), async () => {
        IndexedDbService.getConfiguration = jest.fn()
        IndexedDbService.getConfiguration.mockImplementationOnce(new Error("An error occured"))
...
describe("removeConfigurationFromIndexedDb", () => {
    ...
    it(("calls indexedDb with correct name"), async () => {
      ...
      await removeConfigurationFromIndexedDb({ dispatch, commit, state }, "MamboNo2")
      expect(IndexedDbService.deleteConfiguration).toHaveBeenCalledWith("MamboNo2")
    })
    it(("sets new configuration names"), async () => {
      IndexedDbService.deleteConfiguration = jest.fn()
      await removeConfigurationFromIndexedDb({ dispatch, commit, state }, "MamboNo2")
      expect(commit).toHaveBeenCalledWith("SET_STORED_CONFIGURATION_NAMES", ["GraphNo1"])
    })
    it(("sets success message"), async () => {
      IndexedDbService.deleteConfiguration = jest.fn()
      await removeConfigurationFromIndexedDb({ dispatch, commit, state }, "MamboNo2")
      expect(dispatch).toHaveBeenCalledWith("setSuccess", "Configuration deleted")
    })
    it(("sets error message when IndexedDb errors"), async () => {
      IndexedDbService.deleteConfiguration = jest.fn()
      IndexedDbService.deleteConfiguration.mockImplementationOnce(new Error("An error occured"))
      await removeConfigurationFromIndexedDb({ dispatch, commit, state }, "MamboNo2")
      expect(dispatch).toHaveBeenCalledWith('setError', new Error("An error occured"))
    })
})
...
describe("loadConfiguration",()=>{
    ...
    it("sets success message",()=>{
        ...
        expect(dispatch).toHaveBeenCalledWith("setSuccess","Loaded configuration successfully")
    })
    it("calls SET_CONFIGURATION",()=>{
        loadConfiguration({dispatch, commit},{name: "a config"})
        expect(commit).toHaveBeenCalledWith("SET_CONFIGURATION",{name: "a config"})
    })
})
...
describe("downloadConfiguration", () => {
    ...
    it("sets the download URL", () => {
      ...
      expect(commit).toHaveBeenCalledWith("SET_CONFIGURATION_URL", "blob://out-of-tune.org/123123123123")
    })
  })