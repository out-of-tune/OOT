...
describe("getCurrentUsersPlaylists", () => {
    ...
    it("writes retrieved playlists to the state", async () => {
        ...
        expect(commit).toHaveBeenCalledWith("SET_USER_PLAYLISTS", [{name: "playlist 1"}, {name: "playlist 2"}])
    })
})
...
describe("setCurrentPlaylist", () => {
    ...
    it("sets current playlist", () => {
        ...
        expect(commit).toHaveBeenCalledWith("SET_CURRENT_PLAYLIST", {name: "playlist 1"})
    })
})
describe("addSongToPlaylist",()=>{
    ...
    it("calls add songs to playlist",async ()=>{
        ...
        expect(dispatch).toHaveBeenCalledWith("addSongsToPlaylist", ["SongURIString"])
    })
})
describe("addSongsToPlaylist",()=>{
    ...
    it("adds Songs to playlist",async ()=>{
        ...
        await addSongsToPlaylist({dispatch, rootState, state},["SongURIString", "AnotherSongURI"])
        expect(SpotifyService.addSongsToPlaylist).toHaveBeenCalledWith("TestToken","12345",["SongURIString", "AnotherSongURI"])
    })
    it("sets success message",async ()=>{
        SpotifyService.addSongsToPlaylist = jest.fn()
        SpotifyService.addSongsToPlaylist.mockReturnValue("Success")
        await addSongsToPlaylist({dispatch, rootState, state},["SongURIString", "AnotherSongURI"])
        expect(dispatch).toHaveBeenCalledWith("setMessage","Added 2 songs to sixSevenEight")
    })
    it("errors when no playlist is set",async ()=>{
        state.currentPlaylist.id = undefined
        await addSongsToPlaylist({dispatch, rootState, state},["SongURIString", "AnotherSongURI"])
        expect(dispatch).toHaveBeenCalledWith("setError",new Error("no playlist is chosen"))
    })
    it("errors when no token is provided",async ()=>{
        ...
        expect(dispatch).toHaveBeenCalledWith("setError",new Error("no token provided"))
    })
})
describe("loadPlaylist",()=>{
    ...
    beforeEach(()=>{
        ...
        SpotifyService.getSongsFromPlaylist.mockReturnValue({items:[
            {
                track:{
                    id:"12345",
    ...
    it("calls Spotify API correctly",async ()=>{
        ...
        expect(SpotifyService.getSongsFromPlaylist).toHaveBeenCalledWith("TestToken","12345")
    })
    it("adds song nodes to graph",async ()=>{
        await loadPlaylist({dispatch, rootState, commit}, "12345")
        expect(dispatch).toHaveBeenNthCalledWith(1,"addToGraph",{nodes: [{id:"Song/12345", data: {sid:"12345", metadata: "someMetadata", label: "song"}, links: []}], links:[]})
    })
