import {getAllNodes, getNodePosition, getNodeColor} from "@/assets/js/graphHelper.js"
...
function getDesiredScale(x1, x2, y1, y2){
    ...
    const desiredScale = desiredScaleX>desiredScaleY? desiredScaleY: desiredScaleX
    return desiredScale
}
...
const fitGraphToNodes = ({rootState, commit}, nodes)=>{
    if(nodes.length>0){
        ...
        const desiredScale = getDesiredScale(boundaries.smallestX, boundaries.biggestX, boundaries.smallestY, boundaries.biggestY)
        commit("MOVE_TO",{
            x:boundaries.smallestX+(boundaries.biggestX-boundaries.smallestX)/2, 
            y:boundaries.smallestY+(boundaries.biggestY-boundaries.smallestY)/2
        })
        if(desiredScale && desiredScale != Infinity && desiredScale !=0 && (desiredScale-desiredScale/4)<2){
...
function isNodeOnScreen(nodePosition, graphics){
    ...
    const endPoint = graphics.transformClientToGraphCoordinates({x:window.document.body.clientWidth, y:window.document.body.clientHeight})
    return nodePosition.x > startPoint.x && nodePosition.x < endPoint.x && nodePosition.y > startPoint.y && nodePosition.y < endPoint.y
}
...
const displayNodeLabels = ({rootState, commit})=>{
    const graphics = rootState.mainGraph.renderState.Renderer.getGraphics()
    graphics.placeNode((ui, pos)=> {placeNodeLabel(pos, ui, rootState, commit)})
}
...
export const actions = process.env.NODE_ENV !== 'production'?
{
    moveToNode,
    fitGraphToScreen,
    fitGraphToSelection,
