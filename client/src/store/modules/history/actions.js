...
function categorizeLinks(rootState, newLinks){
    ...
    const allChangedLinks = getAllLinks(rootState).filter(link=>newLinkIds.includes(link.id))
    const links = allChangedLinks.map(link=>{
        const newLinkTypes = newLinks.filter(newLink=>newLink.fromId+"👉 "+newLink.toId===link.id).flatMap(newLink=>newLink.linkTypes)
        const linkTypes = link.linkTypes.filter(linkType=>!newLinkTypes.includes(linkType))
...
function remove(rootState, currentChange, commit){
    ...
    linkCategories.linksToUpdate.forEach(link=>commit("UPDATE_LINKTYPES", link))
}

function add(currentChange, commit, dispatch){
    const newLinks = (currentChange.data.links.length==0) ? currentChange.data.nodes.flatMap(node=>node.links).filter(link=>link!=null) : currentChange.data.links
    commit('ADD_TO_GRAPH', {
        nodes: currentChange.data.nodes, links: newLinks
    })
...
const addChange = ({ state, commit }, change) => {
    const newChanges = [...state.changes.slice(0, state.historyIndex+1), change]
