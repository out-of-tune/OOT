...
describe("setSelectedNodes", () => {
  it("sets the selectedNodes array", () => {
    ...
    expect(commit).toHaveBeenCalledWith("SET_SELECTED_NODES", [
      { id: "node1", data: { label: "Auftrag" } },
      { id: "node2", data: { label: "Auftrag" } },
    ]);
  });
...
describe("selectionFinished", () => {
  ...
  it("sets selection to selected nodes", () => {
    ...
    expect(commit).toHaveBeenCalledWith("SET_TEMPORARY_SELECTED", []);
    expect(dispatch).toHaveBeenNthCalledWith(1, "setSelectedNodes", [
      { id: "a node" },
    ]);
...
describe("setEdgesTransparent", () => {
  ...
  it("sets links transparent that have no connection to selected nodes", () => {
    ...
    expect(dispatch).toHaveBeenNthCalledWith(1, "setLinkOpacity", {
      link: { fromId: "Auftrag/2", toId: "Kunde/1", id: "Auftrag/2👉 Kunde/1" },
      opacity: "44",
    });
  });
  it("sets links half-transparent that have one connection to a selected node", () => {
    ...
    setEdgesTransparent({ commit, rootState, state, dispatch });
    expect(dispatch).toHaveBeenNthCalledWith(2, "setLinkOpacity", {
      link: { fromId: "Auftrag/1", toId: "Kunde/1", id: "Auftrag/1👉 Kunde/1" },
      opacity: "88",
    });
  });
...
describe("removeSelectedNodes", () => {
  ...
  it("sets node opacity", () => {
    state = {
      selectedNodes: [{ id: "Node/1" }, { id: "Node/2" }],
    };
...
describe("invertSelection", () => {
  ...
  it("updates the colors", () => {
    invertSelection({ dispatch, rootState });
    expect(dispatch).toHaveBeenCalledWith("updateSelectionUI", [
      nodes[1],
      nodes[2],
    ]);
  });
...
describe("moveSelection", () => {
  ...
  it("moves the selection", () => {
    moveSelection(
      { commit, rootState },
      {
        originNode: { id: "1" },
        nodesWithPositionToMove,
        oldOriginPosition: { x: 0, y: 0 },
      },
    );
    expect(commit).toHaveBeenCalledWith("SET_NODE_POSITION", {
      nodeId: "2",
      xPosition: 10,
      yPosition: 20,
    });
  ...
  it("doesn't move the dragged node", () => {
    ...
    expect(commit).not.toHaveBeenCalledWith("SET_NODE_POSITION", {
      nodeId: "1",
