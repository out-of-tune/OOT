...
function getNodesWithInBoundaries(rootState, topLeft, bottomRight) {
  ...
  nodes.forEach((node) => {
    ...
    index.add(
      ui.position.x - ui.size / 2,
      ui.position.y - ui.size / 2,
      ui.position.x + ui.size / 2,
      ui.position.y + ui.size / 2,
    );
  });
...
const handleAreaSelected = ({ commit, dispatch, rootState }, { area }) => {
  ...
  if (!_.isEqual(selectedNodes, rootState.selection.temporarySelectedNodes)) {
    if (rootState.selection.temporarySelectedNodes == 0) {
      dispatch(
        "markNodes",
        _.uniq([...rootState.selection.selectedNodes, ...selectedNodes]),
      );
    }
    //get difference between selectedNodes and temporary selected nodes in state
    const addedNodes = selectedNodes
      .filter(
        (node) => !rootState.selection.temporarySelectedNodes.includes(node),
      )
...
function getSelectedLinkIds(nodes) {
  ...
  return nodesWithLinks.flatMap((node) => node.links.map((link) => link.id));
}
...
function categorizeLinks(links, linkCounts) {
  return links.reduce(
    (categorizedLinks, currentLink) => {
      ...
      return {
        ...categorizedLinks,
        [category]: [...categorizedLinks[category], currentLink],
      };
    },
...
const removeSelectedNodes = ({ commit, state, dispatch }) => {
  ...
  dispatch("addChange", {
    data: { nodes: _.cloneDeep(state.selectedNodes), links: [] },
    type: "remove",
  });
...
const addSelectedSongsToQueue = async ({
  commit,
  rootState,
  state,
  dispatch,
  ...
  const updatedNodes = result.map((result, index) => {
    return {
      id: selectedSongNodes[index].id,
      data: {
        ...selectedSongNodes[index].data,
        ...result,
  ...
  dispatch(
    "setSuccess",
    "Added " + selectedSongNodes.length + " songs to queue",
  );
};
...
const invertSelection = ({ rootState, dispatch }) => {
  if (rootState.selection.selectedNodes.length > 0) {
    const selectedSongNodeIds = rootState.selection.selectedNodes.reduce(
      (oldObject, node) => ({ ...oldObject, [node.id]: node.id }),
      {},
    );
    const invertedSelection = getAllNodes(rootState).filter(
      (node) => !selectedSongNodeIds[node.id],
    );
...
const moveSelection = (
  { rootState, commit },
  { originNode, nodesWithPositionToMove, oldOriginPosition },
) => {
