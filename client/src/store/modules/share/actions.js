...
const generateShareLink = async ({ commit, rootState, dispatch }, type) => {
    const object = type === 'graph'
        ? getGraphObject(rootState)
        : rootState.configurations
    const compressed = pako.deflate(JSON.stringify(object), { to: 'string' })
    const response = await handleGraphqlTokenError((ShareService.getShareLink).bind(ShareService), [type, compressed], dispatch, rootState)
    console.log(response)
    commit('SET_SHARE_LINK', `${process.env.VUE_APP_PROXY_URI}/#/graph?uri=${response.uri}&type=${type}`)
}

const importSharedObject = async ({ dispatch, rootState }, { uri, type }) => {
    const binaryString = await handleGraphqlTokenError((ShareService.getSharedObject).bind(ShareService), [uri], dispatch, rootState)
