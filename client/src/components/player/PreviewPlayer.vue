...
/** Tells why the full Spotify player is not in use. */
const placeholder = computed(() => {
  if (!store.state.authentication.loginState)
    return "Click a song node to play a preview. Log in for full songs.";
  switch (store.state.spotify_player.status) {
    case "connecting":
      return "Connecting to Spotify…";
    case "premium_required":
      return "Full songs need Spotify Premium. Click a song node for a preview.";
    case "unavailable":
      return "This browser cannot play Spotify. Click a song node for a preview.";
    default:
      return "Click a song node to play a preview";
  }
});
...
<template>
  ...
  >
    ...
    <div class="min-w-0 sm:w-56">
      <CurrentSongInfo
        :song="store.state.music_player.currentSong"
