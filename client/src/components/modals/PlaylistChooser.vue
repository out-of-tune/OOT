            ...
            <div class="card" >
                ...
                <div class="playlist-box" v-if="loggedIn">
                    ...
                    <div>
                        ...
                        <v-btn small @click="addToPlaylist">add to playlist</v-btn>
                    </div>
                </div>
                <div v-else class="login-box">
                    <div class="content">
                        <h1>You have to be logged in to Spotify to access playlists</h1>
                        <v-btn small v-if="!loggedIn" id="login" outline color="#ffffff" v-on:click="login">login</v-btn>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async addToPlaylist(){
            const songs = this.selectedNodes.filter((node) => node.data.label==="song").map((node)=>node.data.uri)
