    ...
    <div id="searchContainer">
        <div v-if="advanced" id="advanced">
            ...
            <div class="input" id="advancedInput">
                <div id="autocompleteBox" @focusout="changeVisibility" @keydown="selectEvent">
                    <v-text-field class="searchfield" dark type="text" @focus="inputIsFocused=true" v-bind:class="{validString: valid, invalidString: !valid}"
                        ref="searchInput" 
                        name="searchField"
        ...
        <div v-else id="simple">
            ...
            <div class="input" id="simpleInput" @keydown="simpleSearchEvents">
                <v-select
                    ...
                    solo
                ></v-select>
                <v-text-field class="searchfield" dark type="text" name="searchField" id="searchField" v-model="simpleSearchString" >
                    <v-icon 
                        class="searchIcon" 
                        slot="append" 
...
export default {
    methods: {
        ...
        getMatchingData: function(){
            if(this.tip){
                const typesToMatch = this.tip.type==="attribute"? this.getAttributes(this.tip.nodeType) :this.getNodeLabelNames()
        ...
        moveToNext: function(){
            if(this.$refs.autocomplete){
                if(this.selectedItem+1<this.$refs.autocomplete.childNodes.length){
                    ...
                    this.$refs.autocomplete.childNodes[this.selectedItem].scrollIntoView({block: "end", behavior: "smooth"})
        ...
        simpleSearchEvents: function(event){
            if(event.keyCode==13){
                this.startSimpleSearch({nodeType: this.selectedNode, searchString: this.simpleSearchString})
            }
        },
