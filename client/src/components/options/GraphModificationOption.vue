<template>
  <div>
    <div
      class="container"
      v-for="edgeLabel in getEdges(nodeLabel)"
      v-bind:key="edgeLabel"
    >
      <label class="checkbox-container">
        <input
          type="checkbox"
          v-model="selectedExpandOptions"
          v-bind:value="{ nodeLabel: nodeLabel, edgeLabel: edgeLabel }"
          @change="
            updateGraphModificationConfiguration({
              actionType: 'expand',
              selectedOptions: selectedExpandOptions,
              nodeType: nodeLabel,
            })
          "
        />
      ...
      <label class="checkbox-container">
        <input
          type="checkbox"
          v-model="selectedCollapseOptions"
          v-bind:value="{ nodeLabel: nodeLabel, edgeLabel: edgeLabel }"
          @change="
            updateGraphModificationConfiguration({
              actionType: 'collapse',
              selectedOptions: selectedCollapseOptions,
              nodeType: nodeLabel,
            })
          "
        />
...
export default {
  ...
  props: ["nodeLabel"],
  methods: {
    ...mapActions(["updateGraphModificationConfiguration"]),
  ...
  watch: {
    nodeLabel: function () {
      this.selectedExpandOptions = this.convertActionConfigurationToOptions(
        this.$store.state.configurations.actionConfiguration.expand,
      );
      this.selectedCollapseOptions = this.convertActionConfigurationToOptions(
        this.$store.state.configurations.actionConfiguration.collapse,
      );
    },
  },
  mounted: function () {
