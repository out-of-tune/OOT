...
export default {
  ...
  computed: {
    ...mapState({
      ...
      edgeConfiguration: (state) =>
        state.configurations.appearanceConfiguration.edgeConfiguration,
    }),
    edgeRules: {
      get() {
  ...
  methods: {
    ...
    setRules: function () {
      const edgeRule = {
        edgeLabel: this.edgeType.label,
        color: this.formatRuleColor(this.color),
      };
      const newEdgeRules = [
