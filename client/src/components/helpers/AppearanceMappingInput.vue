  ...
  <div id="input">
    ...
    >
      <input
        ...
        v-bind:class="{
          validString: searchObject.valid,
          invalidString: !searchObject.valid,
        }"
        ref="searchInput"
        name="searchField"
        class="searchField"
  ...
  methods: {
    ...
    useSelected: function () {
      if (this.$refs.autocomplete && this.selectedItem != -1) {
        this.addOnClick(
          this.$refs.autocomplete.children[this.selectedItem].innerText,
        );
      } else {
        this.$emit("addRule");
      }
    ...
    addOnClick: function (clickedTooltip) {
      if (this.tip != undefined) {
        if (this.tip.type === "attribute") {
          ...
          this.$emit(
            "update:searchObject",
            generateSearchObject(stringifiedQuery),
          );
        }
      }
    ...
    moveToNext: function () {
      if (this.$refs.autocomplete) {
        if (this.selectedItem + 1 < this.$refs.autocomplete.children.length) {
          ...
          this.$refs.autocomplete.children[this.selectedItem].scrollIntoView(
            false,
          );
        }
      }
    ...
    addCharacterInput: function (searchString) {
      ...
      this.$emit(
        "update:searchObject",
        generateSearchObject(this.searchObject.nodeType + ": " + searchString),
      );
    },
  },
  created: function () {
