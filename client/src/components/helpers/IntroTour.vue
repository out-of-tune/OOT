<template>
  <div>
    <v-dialog v-model="showTour" max-width="40vw" dark>
      <v-card>
        <v-card-title class="headline">Do you want to take a tour through out-of-tune?</v-card-title>

        <v-card-text>
          We prepared a little tutorial for you with all the basic knowledge you need! If you want a more extensive guide visit our
          <router-link :to="{ name: 'Help'}" target="_blank">help page</router-link>.
        </v-card-text>

        <v-card-actions>
          <div class="flex-grow-1"></div>
          <v-btn color="red darken-1" text @click="setShowTour(false)">Skip tour</v-btn>
          <v-btn color="green darken-1" text @click="startTour">Start tour</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
