...
export function getNodesByLabel(nodeLabel, rootState, nodes=[]) {
    ...
    if(affectedNodes.length==0){
        ...
        affectedNodes = affectedNodes.filter(node=>node.data.label===nodeLabel)
    }
...
export function getAllNodes(rootState) {
    let nodes = []
    rootState.mainGraph.Graph.forEachNode(node => { nodes.push(node) })
    return nodes
}
...
export function getNodePosition(rootState, node) {
    return rootState.mainGraph.renderState.Renderer.getGraphics().getNodeUI(node.id).position
}
export function getNodeColor(rootState, node) {
    return rootState.mainGraph.renderState.Renderer.getGraphics().getNodeUI(node.id).color
}
export function getNodeUi(rootState, node){
    return rootState.mainGraph.renderState.Renderer.getGraphics().getNodeUI(node.id)

}

export function getLinkColor(rootState, link) {
    return rootState.mainGraph.renderState.Renderer.getGraphics().getLinkUI(link.id).color
}

export function getAllLinks(rootState) {
    let links = []
    rootState.mainGraph.Graph.forEachLink(link => { links.push(link) })
    return links
}
const convertToRegex = value => new RegExp(value.split("%").join(".*?"))
const like = (nodeData,searchData) =>  nodeData? nodeData.toLowerCase().match(convertToRegex(searchData.toLowerCase())): false
const compare = a => b => compareFn => compareFn(a,b)
function removeQuotes(quotedString){
    return (quotedString[0]==="\"" && quotedString[quotedString.length-1]==="\"")
    ...
    if (searchObject.valid) {
        ...
        const filteredNodes = nodesWithLabel.filter((nodeWithLabel) => {
            const at = attributes.filter(attribute => {
                ...
                const compareNodeAttributeToSearch = compareFn => compare(nodeWithLabel["data"][attributeSearch])(attributeData)(compareFn)
                const compareNodeIdToSearch = compareFn => compare(nodeWithLabel.id)(attributeData)(compareFn)
                const operatorMap = {
                    "=": _.eq,
                    "!=": (a, b) => !_.eq(a, b),
...
export function getGraphObject(rootState){
    ...
    const nodesWithPositions = nodes.map(node=>{
        ...
        return { node: {...nodeData}, position: getNodePosition(rootState, node), pinned: getPinnedState(rootState, node)}
    })
    return {
        nodesWithPositions,
        links: nodeLinks
