...
async function checkNodesExistence(
  nodeLabel,
  sids,
  schema,
  dispatch,
  ...
  const attributes = schema.nodeTypes
    .filter((nodeType) => nodeType.label === nodeLabel)[0]
    .attributes.join(",");
  const queries = sids.map(
    (sid) => `${nodeLabel}(sid: "${sid}"){ ${attributes} }`,
  );
...
export { checkNodesExistence, mergeGraphQlQueries };
