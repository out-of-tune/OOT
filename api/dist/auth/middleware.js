...
const authenticate_user = (arango) => async (ctx) => {
    ...
    if (token && token.startsWith('Bearer ')) {
        ...
        if (!user)
            throw new Error('AuthenticationError: User not found');
        return {
            ...ctx,
            user
        };
...
async function validate_token(token, lookup) {
    ...
    if (!id)
        throw new Error('AuthenticationError: ID is undefined/null, contact an admin or create a bug report'); // probably not necessary anymore
